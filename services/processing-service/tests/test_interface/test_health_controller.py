...
class TestHealthController:
    ...
    async def test_health_check_llm_api_ok(self, async_client: AsyncClient) -> None:
        """Test health check with LLM API OK."""
        # Arrange & Mock
        with patch("src.interface.controllers.health_controller.async_engine") as mock_engine:
            mock_connection = AsyncMock()
            mock_engine.connect.return_value.__aenter__.return_value = mock_connection
            ...
            with patch("src.interface.controllers.health_controller.aio_pika.connect_robust") as mock_pika:
                ...
                with patch("src.interface.controllers.health_controller.httpx.AsyncClient") as mock_http:
                    ...
                    assert response.status_code == 200
                    data = response.json()
                    assert data["dependencies"]["llm_api"] == "ok"

    async def test_health_check_llm_api_degraded_bad_status(self, async_client: AsyncClient) -> None:
        """Test health check with LLM API returning non-200."""
        # Arrange & Mock
        with patch("src.interface.controllers.health_controller.async_engine") as mock_engine:
            mock_connection = AsyncMock()
            mock_engine.connect.return_value.__aenter__.return_value = mock_connection
    ...
    async def test_health_check_llm_api_degraded_exception(self, async_client: AsyncClient) -> None:
        """Test health check with LLM API connection error."""
        # Arrange & Mock
        with patch("src.interface.controllers.health_controller.async_engine") as mock_engine:
            mock_connection = AsyncMock()
            mock_engine.connect.return_value.__aenter__.return_value = mock_connection
